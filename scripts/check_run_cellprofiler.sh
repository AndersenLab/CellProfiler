...
if test -f ${IMAGES}/not_processed.tsv; then
    IMAGE=$(cat ${IMAGES}/not_processed.tsv | head -1 | tail -1); # get the first line.
    echo "$IMAGE" | cat >> ${IMAGES}/not_processed.tsv # add an extra line to fool CP
    ...
    COUNT=1; \
        for i in ===$(seq $N_NOT_PROCESSED); \
            do sbatch ${CPBIN}/scripts/cellprofiler_parallel_bigmem.sh $i 2; \
            let COUNT=$COUNT+1; \
        done
    else
